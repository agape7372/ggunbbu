...
import { GAUGE, PLAYER, STACK } from '../src/config';
...
describe('타격', () => {
  it('타격당 콤보 +1 (층 붕괴당이 아님) + 밀어올림', () => {
    ...
    expect(s.stack!.vy).toBeGreaterThan(0); // HIT_LIFT
  });
...
describe('가드 2종 [정본]', () => {
  it('지면 가드: 높이 띄움 + 콤보 단절 + 게이지 추가 소모', () => {
    ...
    expect(s.stack!.vy).toBeGreaterThan(STACK.GUARD_GROUND_V - 100);
    expect(s.combo).toBe(0);
    expect(s.gauge).toBeLessThan(50 - GAUGE.BOUNCE_COST_GROUND + 1);
  });
  ...
  it('공중 가드: 낮게 띄움 + 콤보 유지', () => {
    ...
    expect(s.stack!.vy).toBeGreaterThan(STACK.GUARD_AIR_V - 100);
    expect(s.combo).toBe(10); // 유지
  });

  it('게이지 미달 시 가드 불가', () => {
    const s = makeState();
    s.gauge = GAUGE.MIN_TO_GUARD - 1;
    withStack(s, 40);
    s.stack!.vy = -100;
    for (let i = 0; i < 10; i++) advance(s, inp({ guard: true }));
...
describe('깔림 3분기 [정본]', () => {
  ...
  it('하: 띄우기 탈출 (라이프 무손실)', () => {
    ...
    expect(s.stack!.vy).toBeGreaterThan(STACK.PIN_ESCAPE_V - 30);
  ...
  it('Z 실패(고HP층): 라이프 추가 손실 + 자비 바운스', () => {
    ...
    expect(s.stack!.vy).toBeGreaterThan(STACK.PIN_MERCY_V - 30);
    expect(s.player.pose).toBe('idle');
  });
});
